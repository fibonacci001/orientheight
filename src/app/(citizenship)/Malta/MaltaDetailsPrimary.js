...
const MaltaDetailsPrimary = () => {
  ...
  useEffect(() => {
    const fetchMaltaDetails = async () => {
      try {
        setLoading(true);
        // Updated to directly reference the Malta document
        const programRef = doc(db, "services", "citizenship", "programs", "Malta");
        const programSnap = await getDoc(programRef);

        if (!programSnap.exists()) {
          setError("Malta program details not found");
          return;
        }
        ...
        setError("Failed to fetch Malta program details");
    ...
    return (
      <div className="project__details sp_top_140 sp_bottom_140">
        <div className="container">
          <div className="text-center">Loading Malta program details...</div>
        </div>
      </div>
    );
  }
  ...
  if (error || !program) {
    return (
      <div className="project__details sp_top_140 sp_bottom_140">
        <div className="container">
          <div className="alert alert-danger">{error || "Malta program not found"}</div>
        </div>
      </div>
    );
  }
...
export default MaltaDetailsPrimary;