...
const StkittsDetailsPrimary = () => {
  ...
  useEffect(() => {
    const fetchstkittsDetails = async () => {
      try {
        setLoading(true);
        // Updated to directly reference the stkitts document
        const programRef = doc(db, "services", "citizenship", "programs", "stkitts");
        const programSnap = await getDoc(programRef);

        if (!programSnap.exists()) {
          setError("stkitts program details not found");
          return;
        }
        ...
        setError("Failed to fetch stkitts program details");
    ...
    return (
      <div className="project__details sp_top_140 sp_bottom_140">
        <div className="container">
          <div className="text-center">Loading stkitts program details...</div>
        </div>
      </div>
    );
  }
  ...
  if (error || !program) {
    return (
      <div className="project__details sp_top_140 sp_bottom_140">
        <div className="container">
          <div className="alert alert-danger">{error || "stkitts program not found"}</div>
        </div>
      </div>
    );
  }
