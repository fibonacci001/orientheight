...
const TurkiyeDetailsPrimary = () => {
  ...
  useEffect(() => {
    const fetchTurkiyeDetails = async () => {
      try {
        setLoading(true);
        // Updated to directly reference the Turkiye document
        const programRef = doc(db, "services", "citizenship", "programs", "Turkiye");
        const programSnap = await getDoc(programRef);

        if (!programSnap.exists()) {
          setError("Turkiye program details not found");
          return;
        }
        ...
        setError("Failed to fetch Turkiye program details");
    ...
    return (
      <div className="project__details sp_top_140 sp_bottom_140">
        <div className="container">
          <div className="text-center">Loading Turkiye program details...</div>
        </div>
      </div>
    );
  }
  ...
  if (error || !program) {
    return (
      <div className="project__details sp_top_140 sp_bottom_140">
        <div className="container">
          <div className="alert alert-danger">{error || "Turkiye program not found"}</div>
        </div>
      </div>
    );
  }
...
export default TurkiyeDetailsPrimary;