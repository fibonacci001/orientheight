...
const UnitedKingdomResidencyDetails = () => {
  ...
  useEffect(() => {
    const fetchUnitedKingdomDetails = async () => {
      try {
        setLoading(true);
        // Updated to directly reference the UnitedKingdom document
        const programRef = doc(db, "services", "Residency", "programs", "UnitedKingdom");
        const programSnap = await getDoc(programRef);

        if (!programSnap.exists()) {
          setError("UnitedKingdom residency program details not found");
          return;
        }
        ...
        setError("Failed to fetch UnitedKingdom residency program details");
    ...
    return (
      <div className="project__details sp_top_140 sp_bottom_140">
        <div className="container">
          <div className="text-center">Loading UnitedKingdom residency program details...</div>
        </div>
      </div>
    );
  }
  ...
  if (error || !program) {
    return (
      <div className="project__details sp_top_140 sp_bottom_140">
        <div className="container">
          <div className="alert alert-danger">{error || "UnitedKingdom residency program not found"}</div>
        </div>
      </div>
    );
  }
...
export default UnitedKingdomResidencyDetails;