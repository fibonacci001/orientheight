...
const ItalyResidencyDetails = () => {
  ...
  useEffect(() => {
    const fetchItalyDetails = async () => {
      try {
        setLoading(true);
        // Updated to directly reference the Italy document
        const programRef = doc(db, "services", "Residency", "programs", "Italy");
        const programSnap = await getDoc(programRef);

        if (!programSnap.exists()) {
          setError("Italy residency program details not found");
          return;
        }
        ...
        setError("Failed to fetch Italy residency program details");
    ...
    return (
      <div className="project__details sp_top_140 sp_bottom_140">
        <div className="container">
          <div className="text-center">Loading Italy residency program details...</div>
        </div>
      </div>
    );
  }
  ...
  if (error || !program) {
    return (
      <div className="project__details sp_top_140 sp_bottom_140">
        <div className="container">
          <div className="alert alert-danger">{error || "Italy residency program not found"}</div>
        </div>
      </div>
    );
  }
...
export default ItalyResidencyDetails;