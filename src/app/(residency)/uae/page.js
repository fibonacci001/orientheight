...
const UAEResidencyDetails = () => {
  ...
  useEffect(() => {
    const fetchUAEDetails = async () => {
      try {
        setLoading(true);
        // Updated to directly reference the UAE document
        const programRef = doc(db, "services", "Residency", "programs", "UAE");
        const programSnap = await getDoc(programRef);

        if (!programSnap.exists()) {
          setError("UAE residency program details not found");
          return;
        }
        ...
        setError("Failed to fetch UAE residency program details");
    ...
    return (
      <div className="project__details sp_top_140 sp_bottom_140">
        <div className="container">
          <div className="text-center">Loading UAE residency program details...</div>
        </div>
      </div>
    );
  }
  ...
  if (error || !program) {
    return (
      <div className="project__details sp_top_140 sp_bottom_140">
        <div className="container">
          <div className="alert alert-danger">{error || "UAE residency program not found"}</div>
        </div>
      </div>
    );
  }
...
export default UAEResidencyDetails;