...
const USAResidencyDetails = () => {
  ...
  useEffect(() => {
    const fetchUSADetails = async () => {
      try {
        setLoading(true);
        // Updated to directly reference the USA document
        const programRef = doc(db, "services", "Residency", "programs", "USA");
        const programSnap = await getDoc(programRef);

        if (!programSnap.exists()) {
          setError("USA residency program details not found");
          return;
        }
        ...
        setError("Failed to fetch USA residency program details");
    ...
    return (
      <div className="project__details sp_top_140 sp_bottom_140">
        <div className="container">
          <div className="text-center">Loading USA residency program details...</div>
        </div>
      </div>
    );
  }
  ...
  if (error || !program) {
    return (
      <div className="project__details sp_top_140 sp_bottom_140">
        <div className="container">
          <div className="alert alert-danger">{error || "USA residency program not found"}</div>
        </div>
      </div>
    );
  }
...
export default USAResidencyDetails;